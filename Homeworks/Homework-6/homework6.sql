...
WHERE mt.LANGUAGE_ID = 'ru' AND ma.ACTOR_ID IN (1, 2) AND m.ID = movie.ID) = 2 AND t.LANGUAGE_ID = 'ru';
...
SELECT ru.ID, IF(ISNULL(en.TITLE), ru.TITLE, en.TITLE) FROM (SELECT * FROM movie
...
INNER JOIN movie_title mt on m.ID = mt.MOVIE_ID
WHERE m.DIRECTOR_ID = 1 AND mt.LANGUAGE_ID = 'ru'
...
WHERE a.ID = actor.ID) MOVIE_COUNT FROM actor;
...
GROUP BY genre.NAME
ORDER BY COUNT(*) DESC LIMIT 1) FROM actor;